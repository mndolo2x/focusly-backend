...
# In-memory document and shared links store for local development/testing without live Supabase database
_in_memory_docs: Dict[str, Dict[str, Any]] = {}
_in_memory_shared_links: Dict[str, Dict[str, Any]] = {}
...
class DocumentService:
    ...
    async def create_share_link(
        self, user_id: str, document_id: str, expires_in_days: int = 7
    ) -> Dict[str, Any]:
        """
        Creates a public share link for a document with 7-day expiration.
        Stores link record in Supabase 'shared_links' or in-memory cache.
        """
        doc = await self.get_document(document_id, user_id)
        if not doc:
            raise ValueError("Document not found or does not belong to user.")
        ...
        _in_memory_shared_links[share_id] = record

        try:
            res = supabase.table("shared_links").insert(record).execute()
            if res.data:
                _in_memory_shared_links[share_id] = res.data[0]
                return res.data[0]
        except Exception:
            pass
    ...
    async def get_public_shared_content(self, share_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieves public content for a share_id if valid and not expired.
        Returns document title, summary sections, and quiz questions (without answers).
        EXCLUDES: video URL, extracted_text, user metadata.
        """
        link = _in_memory_shared_links.get(share_id)
        if not link:
            try:
                res = supabase.table("shared_links").select("*").eq("share_id", share_id).execute()
                if res.data:
                    link = res.data[0]
            except Exception:
                pass
        ...
        title = doc.get("filename", "Shared Learning Content")

        from services.summary_service import summary_service
        from services.quiz_service import quiz_service

        summary = await summary_service.get_latest_summary(doc_id)
        summary_sections = summary.get("sections", []) if summary else []

        questions = await quiz_service.get_quiz_questions(doc_id)
        public_questions = []
        for q in questions:
        ...
        return {
            ...
            "disclaimer": "This public content was generated by AI locally. Please verify important information with primary source materials."
        }

    async def evaluate_public_quiz(
        self, share_id: str, submissions: List[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
        """
        Evaluates quiz submissions for a public share link anonymously without storing user attempt history.
        """
        link = _in_memory_shared_links.get(share_id)
        if not link:
            try:
        ...
        for sub in submissions:
            ...
            selected = sub.get("selected_answer")
            q_obj = q_map.get(q_id)
            if not q_obj:
                continue

            correct = (selected == q_obj.get("correct_answer_index"))
            if correct:
                correct_count += 1

            breakdown.append({
                "question_id": q_id,
                "question_text": q_obj.get("question_text"),
                "selected_answer": selected,
                "correct_answer_index": q_obj.get("correct_answer_index"),
                "is_correct": correct,
            })

        total = len(questions) or len(submissions) or 1
        ...
        return {
            ...
            "breakdown": breakdown,
            "disclaimer": "This content was generated by AI locally. Scores are evaluated anonymously and not saved."
        }
